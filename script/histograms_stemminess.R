...
title <- 'Stemminess (Fiala & Sokal, 1985), N = 1000';

# Calculate min and max empirically
min <- 0;
max <- 0.5;

# binning in 1000 steps for 100 replicates appears to result in 
# no bins having more than ~70 trees in them
steps <- ( max - min ) / 1000;
...
hist(reps$fiala_stemminess, xlim=range(min,max), ylim=range(0,30), breaks = breaks, border = rgb(0,0,1,alpha=0.2), main = mySite, xlab = title);
lines(density(reps$fiala_stemminess, adjust=2), col="blue", lwd=2);

# places an arrow for the true value on the post tree
arrows(true$fiala_stemminess, 3, true$fiala_stemminess, 0.1, length = 0, code = 2, col = 'red', xpd = TRUE, lwd = 2);
points(true$fiala_stemminess, 0.3, pch = 25, col = 'red', bg = 'red');
